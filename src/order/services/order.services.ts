...
export class OrderService {
    ...
    try {
      ...
      return {
        ...
        savedOrder: {
          ...
          orderItems: savedOrder.orderItems.map((order) => ({
            productId: order.product.id,
            name: order.product.name,
            category: order.product.category,
            price: order.orderItemPrice,
            quantity: order.quantity,
            total: order.orderItemTotalAmount
          })),
  ...
  async addNewOrderItem(id: string, dto: CreateOrderDto) {
    try {
      ...
      this.logger.error(
        `Failed to add new order item: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
  ...
  private calculateOrderTotal(orderItems: { orderItemTotalAmount: number }[]) {
    return orderItems.reduce(
      (sum, orderItem) => sum + orderItem.orderItemTotalAmount,
      0,
    );
  }
}
