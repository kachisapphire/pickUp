...
export class AuthService {
    ...
    try {
      ...
      if (!storedCode) {
        throw new NotFoundException(
          'code has expired, please request for a new one',
        );
      }
      ...
      const validate = await this.validateLoginPayload(
        user.id,
        user.email,
        user.role,
      );
  ...
  private generateEmailConfirmationCode() {
  ...
  async login(dto: LoginDto) {
    try {
      ...
      if (!user) {
        throw new NotFoundException(
          'account not found, please create an account or try again',
        );
      }
